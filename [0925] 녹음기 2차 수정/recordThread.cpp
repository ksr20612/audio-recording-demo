...
RecordThread::RecordThread(int chan, int rate){

    mNumChannel = chan; // mono
    mSamplingSize = 2; // 16bit
    mMaximumDuration = 60;
    mSamplingRate = rate; // sampling rate
    mFrames = 32;
    if(mNumChannel==2){
        mFrames*=2;
    }
...
void RecordThread::run(){
    ...
    if ((err = snd_pcm_hw_params_set_format (capture_handle, hw_params, SND_PCM_FORMAT_S16_LE)) < 0) {
      fprintf (stderr, "fail to set sampling size (%s)\n",
               snd_strerror (err));
      exit (1);
    }
    ...
    if ((err = snd_pcm_hw_params_set_channels (capture_handle, hw_params, mNumChannel)) < 0) {
...
//    cin >> mfileName;
    while(*mfileName == NULL){ usleep(1); }
    char *extension = ".wav";
    strcat(mfileName, extension);

    WavWriter wr;
    wr.writeWav(mfileName,numLoop*mFrames,mBuffer,mSamplingRate,mNumChannel,mSamplingSize);
